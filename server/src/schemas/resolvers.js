        ...
        saveBook: async (_parent, { input }, context) => {
            if (context.user) {
                ...
                try {
                    ...
                    const updatedUser = await User.findByIdAndUpdate(context.user._id, { $addToSet: { savedBooks: book._id } }, { new: true, runValidators: true }).populate('savedBooks');
        ...
        removeBook: async (_parent, { bookId }, context) => {
            if (context.user) {
                try {
                    ...
                    const updatedUser = await User.findByIdAndUpdate(context.user._id, { $pull: { savedBooks: book._id } }, { new: true }).populate('savedBooks');
                    if (!updatedUser) {
                        throw new Error('User not found');
                    }
